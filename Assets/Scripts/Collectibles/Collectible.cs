...
public class Collectible : MonoBehaviour
{
    ...
    public void DetermineMines()
    {
        ...
        {
            PowerUp handler = shuffledTiles[i].GetComponent<PowerUp>();
            handler.SetMine();
        }
    }
    ...
    public void SetMinesTouching()
    {
        for (int i = 0; i < tiles.Count; i += 1)
        {
            ...
            tiles[i].GetComponent<PowerUp>().SetNumMinesTouching(mineCount);
        }
    }
    ...
    void FlipAllTiles()
    {
        for (int i = 0; i < tiles.Count; i += 1)
        {
            tiles[i].GetComponent<PowerUp>().FlipTile();
        }
    }
    ...
    public int CheckAboveBelow(int tileIdx, bool checkAbove, bool checkBelow)
    {
        ...
        if (checkAbove)
        {
            if (tiles[tileIdx - colCount].GetComponent<PowerUp>().IsMine())
            {
    ...
    {
        int minesTouchingCount = 0;
        if (tiles[tileIdx - 1].GetComponent<PowerUp>().IsMine())
        {
            minesTouchingCount += 1;
        }
